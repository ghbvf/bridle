...
loop:
    ...
    6. apply remediation, 按可逆性 R0-R5 让渡：
         ...
         R4 (影响用户): 阻塞 + 强制人决策
         R5 (资金 / 物理): 永不放开
    7. report status to dashboard + 决策审计存储
    8. 状态变化反馈回 L1 · 下一轮的输入
```
...
> **本节回答**：编队作战让 v7 的"AI 单兵"演化为"AI 集群"——这是 §10 R 梯度的并行版本：**同一可逆动作，多 agent 抢跑择优**。
>
...
#### reconcile 决策点：patch 还是 replace
...
> **本节回答**：模块的可替换性是 v7 之后的新工程经济学。**框架是机床、模块是商品** —— 商品坏了就换，机床不动。这是 §4.1 资本论 C/V 视角的自然推论。
